...
## PHASE 5A — CORE SERVICE MONEY PAGES — COMPLETE

2026-09-21: **локалната реализация е завършена и проверена; няма commit, push или deploy за тази фаза.** Изходната точка е `514fe42`, която вече е била публикувана преди старта на Фаза 5A. Фаза 5B не е започната.

### Обхват и промени

- Преработени са само съществуващите URL `/repatrirane.html`, `/podavane-na-tok.html` и `/smqna-na-guma.html`; не са създавани нови публични service, city или blog страници и няма URL миграция.
- Запазена е оригиналната responsive структура и общият дизайн. `assets/css/style.css` и `assets/js/main.js` не са променяни. Повторно са използвани съществуващите `home.css`/`home.js` само за opt-in CTA „Изпрати местоположение“.
- Всеки URL вече има един ясен intent, уникални title/description/H1, процес, необходима информация, ценова логика без измислени цени, конкретен CTA, естествена връзка към Добрич и вътрешни връзки към техника/контакт/релевантни услуги.
- Премахнати са непотвърдени обещания за 30 минути, универсално обслужване на всички типове превозни средства, конкретни капацитети и автоматично приложим широк географски обхват от съдържанието на трите страници.
- Репатриране: ясно разграничение от планиран транспорт; информация за колела, управление, скоростна кутия, достъп и дестинация; условна преценка при блокирали колела/автоматик.
- Подаване на ток: разграничено от диагностика и гарантирано запалване; описани признаци, ограничения, предупреждения при видима повреда/теч/дим и възможна следваща стъпка репатриране.
- Спукана гума: смяна на място само след потвърждение и при годна подходяща резервна гума; не се обещават доставка, лепене, баланс или мобилен гумаджийски ремонт; репатриране при неподходящи условия.
- Използвани са само налични реални фирмени изображения. Страницата за репатриране вече използва `platforma-transportni-uslugi-sisi.jpg`; другите две използват `jalt-kamion-patna-pomosht-sisi.jpg` с фактологичен alt, без да представят снимката като конкретно извършена услуга.
- Всеки URL има един JSON-LD `@graph`: `Service` → централния business `https://www.putnapomoshtsisi.com/#business`, `BreadcrumbList` и един `FAQPage`, чиито въпроси и отговори съвпадат с видимия FAQ.
- Добавен е непубличен `SERVICE_CASES`/`OWNER_VERIFICATION_REQUIRED` маркер; реални случаи ще се публикуват само след проверени данни и разрешени снимки. Sitemap вече съдържаше трите URL и не е променян.
- В началната service карта е уточнен anchor-ът „Помощ при спукана гума“; Dobrich page вече съдържаше естествени връзки към трите услуги и не е променяна.

### Проверки

- PASS: `node tools/check-core-services.mjs` — 3/3 URL; metadata, self-canonical, index/follow, един H1, Service/Breadcrumb/FAQ schema, central provider `@id`, видимо FAQ съвпадение, phone/location CTA, вътрешни цели и sitemap.
- PASS: `python3 tools/check-seo.py` — 30 страници, 0 нови грешки; остават 7 известни duplicate FAQ находки на други местни страници извън Фаза 5A.
- PASS: JS syntax за `home.js`, `main.js` и новия checker; `git diff --check`.
- PASS: локален HTTP — трите URL връщат 200 и byte-for-byte съвпадат с файловете на диска.
- PASS: Safari desktop — визуално проверени трите hero/content изгледа, изображения, breadcrumbs и CTA; location диалогът се отваря и изрично казва, че нищо не се изпраща автоматично.
- PASS: Safari mobile QA чрез временни same-origin iframe viewport-и — 386 px client/scroll width за 3/3, 0 хоризонтално преливане, 0 счупени eager изображения, mobile menu и sticky контактни действия налични. Временният QA файл е премахнат.
- Няма build script — сайтът е статичен. Не е извършван външен Schema Validator/Rich Results тест, Lighthouse/CWV измерване, реално обаждане, SMS или геолокация.

### OWNER INPUT — остава за проверка

- Репатриране: точни типове превозни средства; максимална маса/капацитет; реална процедура при автоматик и блокирали колела; възможности/ограничения на лебедката; проверени случаи и разрешени снимки.
- Подаване на ток: точен оперативен процес; поддържани типове автомобили/системи; ограничения; какво реално се предлага, ако автомобилът не запали.
- Спукана гума: задължителна ли е резервна гума; извършва ли се реална смяна на пътя; предлага ли се репатриране до гумаджийница; ограничения.
- Общо: реална ценова логика; актуално покритие; потвърждение за текущо 24/7 приемане на заявки; проверена политика за време на реакция; снимки с потвърдени права и контекст.

### Канибализация и препоръчана следваща фаза

- `/patna-pomosht-dobrich.html` остава собственик на общия city intent „пътна помощ Добрич“ и локалния „репатрак Добрич“. `/repatrirane.html` е ограничена до самата услуга, процеса и репатриране на автомобил, без „Добрич“ в title/H1. Това намалява, но не доказва напълно премахната канибализация без GSC query×page данни.
- `/podavane-na-tok.html` и `/smqna-na-guma.html` притежават конкретните service+city intents; Dobrich страницата ги представя само като кратки service избори с естествени линкове. Началната страница остава regional/brand hub.
- Не са добавяни повторени градски параграфи. Останалите стари city/service страници могат да съдържат исторически overlap и се разглеждат само в отделно одобрена фаза.
- Препоръчана следваща стъпка: owner фактологичен преглед на трите страници, след което отделно одобрение за Фаза 5B върху следващата малка група service money pages. Преди production release: commit/preview, повторен mobile/desktop QA и изрично одобрение за deploy.

**STOP след Фаза 5A. Не е започвана Фаза 5B и текущите промени не са публикувани.**
